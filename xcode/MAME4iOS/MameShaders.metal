                ...
                constant MameScreenTestUniforms &uniforms [[buffer(0)]])
{
    ...
    float2 uv = float2(v.tex.x + cos(t) * uniforms.factor_u * (1.0 / texture.get_width()),
                       v.tex.y + sin(t) * uniforms.factor_v * (1.0 / texture.get_height()));
...
{
    float2 uv = v.tex * float2(texture.get_width(), texture.get_height());
